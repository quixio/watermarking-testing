...
1. **`idle_watermark` initialisation**: added `idle_watermark = None` before the `while` loop in `app.py` to fix `UnboundLocalError` crash on startup (manifested as CB=0 with 4 partitions and 4 replicas).
2. **Topology updated to 4 partitions/replicas**: `run_test.py` creates `highway-2` with 4 partitions; `compose.local.yaml` uses `deploy.replicas: 4` for both CB and CBNWM; watermarks topic remains 1 partition.
